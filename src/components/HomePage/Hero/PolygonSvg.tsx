...
const PolygonSvg: React.FC<Props> = ({ text, onClick }) => {
  ...
  return (
    <div
      className="relative group flex justify-center items-center hover:text-white cursor-default" // Added fixed width and height
      onClick={() => {
        if (onClick) {
          onClick(text);
        }
    ...
    >
      ...
      <svg
        fill="none"
        className="fill-white transition-all w-[94px] h-[94px] relative group-hover:scale-[1.45] duration-1000 group-hover:fill-[#1196CD] group-hover:z-[100]"
      ...
      <svg
        fill="none"
        xmlns="http://www.w3.org/2000/svg"
        className="absolute stroke-[#88CCE7] top-[6%] left-[6%] w-[94px] h-[94px] group-hover:stroke-white group-hover:scale-[1.45] transition-all group-hover:z-[100] duration-1000"
        viewBox="0 0 120 100"
        // style={{
        //   top: "6%",
        //   left: "6%",
      ...
      <div className="absolute w-full h-full top-[3px] flex justify-center items-center left-[-1px] px-5">
        <Text className="text-[10px] font-light group-hover:text-white transition-all group-hover:scale-[1.45] group-hover:z-[100] duration-1000 text-center text-black">
          {text}
        </Text>
      </div>
    </div>
